...
== NAME
display_config, display_blank, display_charblock, display_screenblock,
display_raster, display_get_page, display_set_page - display functions
...
**void display_blank(bool **__enable__**);**
...
is allocated. This function also disables backgrounds and windows, sets
the raster page to 0, and enables forced blank.

Video modes 0-2 use tiled backgrounds (tilemaps). Which backgrounds are
available and their type (regular or affine) depend on the video mode
selected. Charblocks are allocated as follows: 0-3 background tilesets,
4-5 sprite tileset.
...
The *display_blank*() function enables or disables forced blank, based
on the value of _enable_. When enabled, all pixels are set to white.
