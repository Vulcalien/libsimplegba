...
== NAME
timer_config, timer_start, timer_stop, timer_restart, timer_counter
- hardware timers
...
that timer is raised, if enabled: see *interrupt_toggle*(3).

=== Timer configuration
The *timer_config*() function stops the timer and configures it. The
...
The *timer_counter*() function returns the timer's counter value. Since
stopping a timer does not affect its counter, it is safe to call this
function on stopped timers.

== RETURN VALUE
The *timer_counter*() function returns the timer's counter value, an
