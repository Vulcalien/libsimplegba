...
_libsimplegba_ is a utility library for the _Game Boy Advance_ that
provides interfaces to hardware components, debug tools, compilation
files and more.

== Modules
The library is made up of many _modules_, each handling a specific area
...
.4+^.^|Cartridge
|Cartridge  |game cartridge (e.g. header)
...
== Building
Make sure the _arm-none-eabi-gcc_ compiler and the _GNU make_ tool.
...
_Game Boy Advance_ ROMs need a header, which contains metadata for the
program. Copy the template from `files/header.s` into your source code
directory, then change _Game Title_, _Game Code_ and (optionally)
_Software Version_.

After making these changes, you'll need to update the _Header Checksum_.
To do that, compile the ROM and run the `tools/verify-rom` script on it.
Use the checksum that the script prints, then verify the ROM again.
...
## License
_libsimplegba_ is released under the GNU General Public License, either
version 3 of the License or any later version. Any work that uses this
library must be released under a
link:https://www.gnu.org/licenses/license-list.html[GPL-Compatible Free
Software license].
